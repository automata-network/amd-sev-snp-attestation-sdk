...
contract SEVAgentTest is RiscZeroGroth16Setup, SuccinctGroth16Setup, PicoGroth16Setup {
    ...
    string internal constant PROGRAM_ID_PATH = ".program_id.verifier_id";
    string internal picoInputJson = vm.readFile(string.concat(vm.projectRoot(), "/test/assets/pico_azure.json"));
    ...
    function _fixtureProgramId(string memory json) internal pure returns (bytes32) {
        return vm.parseJsonBytes32(json, PROGRAM_ID_PATH);
    }
    ...
    function setUp() public override {
        ...
        ZkCoProcessorConfig memory riscZeroConfig = ZkCoProcessorConfig({
            latestProgramIdentifier: _fixtureProgramId(riscZeroJson), defaultZkVerifier: address(riscZeroVerifier)
        });
        ...
        ZkCoProcessorConfig memory sp1Config =
            ZkCoProcessorConfig({latestProgramIdentifier: _fixtureProgramId(sp1Json), defaultZkVerifier: sp1Verifier});
        ...
        ZkCoProcessorConfig memory picoConfig =
            ZkCoProcessorConfig({latestProgramIdentifier: _fixtureProgramId(picoInputJson), defaultZkVerifier: picoVerifier});
    ...
    function testSevAttestationPico() public {
        ...
        bytes memory programOutput = abi.decode(vm.parseJson(picoInputJson, ".raw_proof.journal"), (bytes));
        bytes memory proof = abi.decode(vm.parseJson(picoInputJson, ".onchain_proof"), (bytes));

        VerifierJournal memory journal =
            attestation.verifyAndAttestWithZKProof(programOutput, ZkCoProcessorType.Pico, proof);

        assertEq(uint8(journal.result), uint8(VerificationResult.Success));
    }

    function testConfiguredProgramIdentifiersMatchFixtures() public view {
        assertEq(attestation.programIdentifier(ZkCoProcessorType.RiscZero), _fixtureProgramId(riscZeroJson));
        assertEq(attestation.programIdentifier(ZkCoProcessorType.Succinct), _fixtureProgramId(sp1Json));
        assertEq(attestation.programIdentifier(ZkCoProcessorType.Pico), _fixtureProgramId(picoInputJson));
    }
}
